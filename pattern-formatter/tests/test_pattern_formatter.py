...
class TestPatternEngine:
    ...
    def test_prominent_heading_detection(self):
        """Test prominent headings (bold, stars, underlined, ALL CAPS)"""
        ColorPrint.header("TEST 3A: Prominent Heading Detection")
        cases = [
            ("**Important Notice**", True, 'prominent_heading'),
            ("**Warning:** System maintenance", True, 'prominent_heading'),
            ("***CRITICAL UPDATE***", True, 'prominent_heading'),
            ("KEY POINTS", True, 'prominent_heading'),
            ("**1)** First Priority**", True, 'prominent_heading'),
            ("MAIN OBJECTIVES", True, 'prominent_heading'),
            ("This is **bold text** within a paragraph.", False, 'paragraph'),
            ("**", False, 'paragraph'),
            ("TOOLONGTOBEHEADINGBECAUSEITEXCEEDSTHIRTYCHARACTERS", False, 'paragraph'),
        ]

        for line, should_be_prominent, expected_type in cases:
            # For the underlined case, provide prev/next lines
            if line.strip() == 'Text between underlines':
                prev_line = '_____'
                next_line = '_____'
                result = self.engine.analyze_line(line, 0, prev_line=prev_line, next_line=next_line)
            else:
                result = self.engine.analyze_line(line, 0)

            is_prominent = result['type'] == expected_type
            if is_prominent == should_be_prominent:
                ColorPrint.success(f"'{line[:40]}' → {'Prominent' if is_prominent else 'Not Prominent'}")
                self.passed += 1
            else:
                ColorPrint.error(f"'{line[:40]}' → Got {result['type']} expected {'prominent_heading' if should_be_prominent else 'non-prominent'}")
                self.failed += 1

    def test_enhanced_numbering_detection(self):
        """Test enhanced hierarchical numbering detection"""
        ColorPrint.header("TEST 3B: Enhanced Numbering Detection")
        cases = [
            ("A.1.2 Specific detail", True, 'hierarchical_list'),
            ("(a) Sub-point", True, 'hierarchical_list'),
            ("((3)) Important note", True, 'hierarchical_list'),
            ("[2] Alternative method", True, 'hierarchical_list'),
            ("1- Introduction", True, 'hierarchical_list'),
            ("II... Second major point", True, 'hierarchical_list'),
            ("1. Normal list item", False, 'numbered_list'),
            ("A) Simple letter list", False, 'numbered_list'),
        ]

        for line, should_match, expected_type in cases:
            result = self.engine.analyze_line(line, 0)
            matches = (result['type'] == expected_type)
            if matches == should_match:
                ColorPrint.success(f"'{line[:40]}' → {'Matched' if matches else 'Not matched'}")
                self.passed += 1
            else:
                ColorPrint.error(f"'{line[:40]}' → Got {result['type']} expected {expected_type}")
                self.failed += 1

    def test_shortdoc_header_detection(self):
        """Test short document (assignment) header detection"""
        ColorPrint.header("TEST 3C: Short Document Header Detection")
        cases = [
            ("1. Introduction", True, 'shortdoc_header'),
            ("Q1: What is Python?", True, 'shortdoc_header'),
            ("Task A: Install software", True, 'shortdoc_header'),
            ("Part B: Methodology", True, 'shortdoc_header'),
            ("Requirement 2.1: Security", True, 'shortdoc_header'),
            ("Step 3: Test the system", True, 'shortdoc_header'),
            ("1. Introduction to the topic which is very long.", False, 'paragraph'),
            ("Q:", False, 'paragraph'),
            ("Task: Do something", False, 'paragraph'),
        ]
    ...
    def test_no_false_positives_from_prose(self):
        """Ensure regular text is NOT detected as tables"""
        ColorPrint.header("TEST 8.5: No False Positives from Prose")
        
        # These should ALL be paragraphs, NOT tables
        false_positive_examples = [
            "Cloud computing has fundamentally transformed enterprise IT",
            "The global cloud market reached $912.77 billion in 2025",
            "Organizations reduce IT expenditure by leveraging shared infrastructure",
            "This transformation is significant for multiple reasons",
            "Traditional on-premises environments required substantial capital investment",
            "explain economies of scale principles in cloud computing contexts",
            "analyze cloud production models and underlying infrastructure",
            "Technical Economies arise from large-scale production methods",
            "Purchasing Economies emerge from provider scale",
            "Cloud providers achieve significant advantages through resource",
        ]
        
        for text in false_positive_examples:
            result = self.engine.analyze_line(text, 0)
            if result['type'] != 'paragraph':
                ColorPrint.error(f"FALSE POSITIVE: '{text}' detected as {result['type']}")
                self.failed += 1
            else:
                ColorPrint.success(f"'{text}' correctly identified as paragraph")
                self.passed += 1
    
    def test_real_tables_still_detected(self):
        """Ensure real tables ARE still detected"""
        ColorPrint.header("TEST 8.6: Real Tables Still Detected")
        ...
        real_tables = [
            ...
            # Numeric table
            ("100    200    300", 'plain_table_row'),
            ("1.5    2.75   3.0", 'plain_table_row'),
            
            # Clear data table
            ("ID     Name     Age", 'plain_table_header'),
            ("1      John     25", 'plain_table_row'),
            
            # Tab-separated
            ("Name\tAge\tCity", 'plain_table_row'),
            
            # Pipe-delimited
            ("A | B | C", 'plain_table_row'),
        ]
        
        for text, expected_type in real_tables:
            result = self.engine.analyze_line(text, 0)
            if result['type'].startswith('plain_table'):
                ColorPrint.success(f"'{text}' correctly detected as table")
                self.passed += 1
            else:
                ColorPrint.error(f"FALSE NEGATIVE: '{text}' not detected as table (got {result['type']})")
                self.failed += 1
    
    def test_prose_not_detected_as_tables(self):
        """Ensure ALL prose is correctly identified as paragraphs"""
        ColorPrint.header("TEST 8.5: No False Positives from Prose")
        
        prose_examples = [
            # From your actual document
            "Cloud computing has fundamentally transformed enterprise IT economics",
            "The global cloud market reached $912.77 billion in 2025",
            "Organizations reduce IT expenditure by leveraging shared infrastructure",
            "This transformation is significant for multiple reasons",
            "Traditional on-premises environments required substantial capital investment",
        ...
        for text in prose_examples:
            result = self.engine.analyze_line(text)
            if result['type'] != 'paragraph':
                ColorPrint.error(f"FALSE POSITIVE: '{text}' detected as {result['type']}")
                self.failed += 1
            else:
    ...
    def run_all_tests(self):
        ...
        self.test_prominent_heading_detection()
        ...
        self.test_no_false_positives_from_prose()
        self.test_real_tables_still_detected()
