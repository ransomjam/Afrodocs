...
def main():
    print("=" * 60)
    print("DEBUG: Page Break Source Analysis")
    print("=" * 60)
    
    processor = DocumentProcessor()
    result, _ = processor.process_text(sample)
    
    analyzed = result.get('analyzed', [])
    structured = result.get('structured', [])
    
    print(f"\nAnalyzed lines with needs_page_break=True:")
    for i, line in enumerate(analyzed):
        if line.get('needs_page_break'):
            print(f"  Line {i}: type={line.get('type')}, content={line.get('content', '')[:50]!r}")
    
    print(f"\nStructured sections with page break flags:")
    for i, s in enumerate(structured):
        heading = s.get('title', s.get('heading', 'N/A'))[:40]
        npb = s.get('needs_page_break', False)
        sonp = s.get('start_on_new_page', False)
        typ = s.get('type', 'unknown')
        if npb or sonp:
            print(f"  [{i}] type={typ}, heading={heading!r}")
            print(f"       needs_page_break={npb}, start_on_new_page={sonp}")
    
    # Now generate document and check for actual page breaks
    print("\n" + "=" * 60)
    print("Generating document...")
    print("=" * 60)
    
    output_path = os.path.join(os.path.dirname(__file__), 'outputs', 'debug_page_break_test.docx')
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    generator = WordGenerator()
    generator.generate(structured, output_path, images=[])
    
    print(f"is_short_document: {generator.is_short_document}")
    print(f"Generated: {output_path}")
    
    # Check for page breaks in output
    ...
    for i, para in enumerate(doc.paragraphs):
        for elem in para._element.iter():
            if elem.tag == qn('w:br'):
                ...
                if br_type == 'page':
                    page_break_count += 1
                    text = para.text[:30] if para.text else '[empty]'
                    print(f"  PAGE BREAK at para {i}: {text!r}")
