...
print(f"\nProcessing {len(lines)} lines of document content...")
print("\n" + "-" * 80)
...
for i, line in enumerate(lines, 1):
    ...
    print(f"Line {i:2d} {symbol}  {display}")

print("\n" + "-" * 80)
print("CLASSIFICATION SUMMARY:")
print("-" * 80)
print(f"Bold numeric headers (1., 2., 3.):     {len(classifications['bold_numeric'])} items")
print(f"Subsection letters (a., b., c.):       {len(classifications['subsection_letter'])} items")
print(f"Plain text:                             {len(classifications['plain_text'])} items")
print(f"Empty lines:                            {len(classifications['empty'])} items")
...
for line in lines:
    ...
    if re.search(r'^\d+\.\s+\*\*\d+\.', stripped):
        issues_found.append(f"Double numeric numbering: {stripped}")
    if re.search(r'^\d+\.\s+\*\*[a-z]\.', stripped):
        issues_found.append(f"Number + letter numbering: {stripped}")
    if re.search(r'^\*\*\d+\.\s+\d+\.', stripped):
        issues_found.append(f"Double bold numeric: {stripped}")
...
if issues_found:
    ...
    print("✅ Document structure looks correct")
...
1. Bold numbered sections: **1.**, **2.**, **3.**
   Status: ✅ VALID - These should stay as-is (bold headers)
   
2. Lettered subsections: a., b., c.
   Status: ✅ VALID - These are sub-items with letter numbering
   
3. Body text paragraphs
   Status: ✅ NORMAL - Plain content between sections
...
print("✅ No double-numbering issues detected")
print("✅ Numbering hierarchy is maintained")
